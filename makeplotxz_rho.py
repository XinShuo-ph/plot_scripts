...
for itidx in range(len(datasets)):
    ...
    slc = yt.SlicePlot(ds, 'y', ('gas', 'scalar_rho'))
    if not plot_log:
        slc.set_log(('gas', 'scalar_rho'), False)

    slc.set_width((extent_x, extent_y))
    if extent_x>100:
        slc.set_xlabel('$z/100M$')
        slc.set_ylabel('$x/100M$')
    else:
        slc.set_xlabel('$z/M$')
        slc.set_ylabel('$x/M$')
    ...
    if labelT:
        ...
        slc.annotate_text((0.05, 0.95), "$t = %.2f M$" % (float(ds.current_time)), coord_system='axis',
                      text_args={'color': 'black', 'fontsize': plot_fontsize})
    filename_base = f"rho_xz_ext{extent_x:.1f}_it{itidx:05d}"
...
imageio.mimsave(os.path.join(plotdir, simname, f"rho_xz_ext{extent_x:.1f}.gif"), images, duration=0.1)
...
for itidx in range(len(datasets)):
    ...
    slc = yt.SlicePlot(ds, 'y', ('gas', 'scalar_rho2'))
    if not plot_log:
    ...
    filename_base = f"rho2_xz_ext{extent_x:.1f}_it{itidx:05d}"
    slc.save(os.path.join(outputdir, f"{filename_base}.png"))
    slc.save(os.path.join(outputdir, f"{filename_base}.pdf"))
    allfiles.append(os.path.join(outputdir, f"{filename_base}.png"))
...
for filename in allfiles:
    images.append(imageio.imread(filename))
imageio.mimsave(os.path.join(plotdir, simname, f"rho2_xz_ext{extent_x:.1f}.gif"), images, duration=0.1)
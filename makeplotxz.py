...
for plotfield in fields_toplot:
    ...
    for itidx in range(len(datasets)):
        ...
        slc = yt.SlicePlot(ds, 'y', plotfield)
        if not plot_log:
        ...
        if labelT:
            ...
            filename_base = f"{plotfield}_xz_ext{extent_x:.1f}_T_it{itidx:05d}"
        else:
            slc.annotate_text((0.05, 0.95), "$t = %.2f M$" % (float(ds.current_time)), coord_system='axis',
                        text_args={'color': 'black', 'fontsize': plot_fontsize})
            filename_base = f"{plotfield}_xz_ext{extent_x:.1f}_it{itidx:05d}"
    ...
    imageio.mimsave(os.path.join(plotdir, simname, f"{plotfield}_xz_ext{extent_x:.1f}.gif"), images, duration=0.1)